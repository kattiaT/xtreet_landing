...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-white/10 backdrop-blur-sm text-white">
...
>
  <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    />
        ...
        <nav className="hidden md:flex gap-10 font-semibold text-lg">
      ...
      {menuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#31004f]/95 backdrop-blur-md py-6 px-8 space-y-4 text-white z-40">
          <Link to="/" className="block text-lg font-medium" onClick={() => setMenuOpen(false)}>Inicio</Link>
          <a href="#coleccion" className="block text-lg font-medium" onClick={() => setMenuOpen(false)}>Colección</a>
          <a href="#galeria" className="block text-lg font-medium" onClick={() => setMenuOpen(false)}>Galería</a>
          <Link to="/Contacto" className="block text-lg font-medium" onClick={() => setMenuOpen(false)}>Contacto</Link>
        </div>
      )}
    </header>
  );
